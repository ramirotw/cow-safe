...
import {
  GPv2Settlement as settlementAddresses,
  GPv2VaultRelayer as vaultAddresses,
...
import {
  ...
  getSigner,
  getExplorerUrl,
  getGnosisSafeServiceUrl,
  getSafeNetworkShortname,
  confirm,
} from './utils'
import {
...
function printExplorer(orderId: string, fromAccount: string, chainId: ChainId) {
  ...
  )}. See ${chalk.blue(`${cowExplorerUrl}/orders/${orderId}`)}
              See ${chalk.underline('full history')} in ${chalk.blue(
    `${cowExplorerUrl}/address/${fromAccount}`
  )}`)
}
...
async function run() {
  ...
  if (myArgs.length === 0) {
    console.error(
      `${chalk.cyan(
        'Missing argument. Path to the order definition JSON file'
      )}. i.e. yarn sell examples/eoa-rinkeby-market-order.json`
    )
    exit(99)
  }
  ...
  console.log(
    `${chalk.red('CoW SDK')} ${chalk.cyan(
      'initialized'
    )}. Signing Account: ${chalk.blue(
      signingAccount ? signingAccount : 'Undefined'
    )}, Network: ${chalk.blue(chainId)}`
  )
  ...
  if (accountType === 'EOA') {
    ...
    assert(
      safeAddress,
      `The safeAddress is a required parameter for account type: ${account.accountType}`
    )
  ...
  // Get quote
  console.log(
    `${chalk.cyan('Get quote for order')}:\n${JSON.stringify(
      quoteOrder,
      null,
      2
    )}`
  ...
  console.log(
    `${chalk.cyan('Quote response')}: Receive at least ${chalk.blue(
      buyAmount
    )} buy tokens. Fee = ${chalk.blue(feeAmount)}\n${JSON.stringify(
      quoteResponse,
      null,
      2
    )} sell tokens.`
  ...
  const buyAmountAfterSlippage = BigNumber.from(buyAmount)
    .mul(TEN_THOUSAND.sub(BigNumber.from(slippageToleranceBips)))
    .div(TEN_THOUSAND)
  console.log(
    `${chalk.cyan(
      `Apply ${chalk.blue(
        slippageToleranceBips + ' BIPs'
      )} to expected receive tokens`
    )}. Accepting ${chalk.blue(buyAmountAfterSlippage)}, expected ${chalk.blue(
      buyAmount
    )}`
  )
  ...
  if (sellBalance.lt(sellAmountBeforeFee)) {
    throw new Error(
      `User doesn't have enough balance of the sell token. Required ${sellAmountBeforeFee}, balance ${sellBalance}`
    )
  }
  ...
  if (allowance.lt(sellAmount)) {
    ...
    txs.push({
      ...
      txRequest: {
        to: sellTokenAddress,
        value: '0',
        data: sellToken.interface.encodeFunctionData('approve', [
          vaultAddress,
          MAX_U32,
        ]),
      },
  ...
  if (account.accountType === 'EOA') {
    ...
    if (txsTotal > 0) {
      console.log(
        `\n\n${chalk.cyan(
          `${chalk.red(txsTotal)} transactions need to be executed`
        )} before the order can be posted:\n`
      )
      let txNumber = 1
      for (const { txRequest, description } of txs) {
        const { to, data } = txRequest
        console.log(
          `    [${txNumber}/${txsTotal}] ${chalk.cyan(
        ...
        if (sendTransaction) {
          ...
          console.log(
            `    Sent transaction for ${chalk.blue(
              description
            )}. Review in block explorer: ${chalk.blue(
              getExplorerUrl(chainId) + '/' + txResponse.hash
            )}`
          )
          await txResponse.wait()
          console.log(
            `    🎉 ${chalk.cyan(
              'Transactions was mined!'
            )} waiting for ${chalk.red(
              NUMBER_CONFIRMATIONS_WAIT
            )} confirmations before continuing`
          )
    ...
    const postOrder = await confirm(
      `${chalk.cyan('Are you sure you want to post this order?')}`
    )
    if (postOrder) {
      // Sign the order
      ...
      console.log(
        `${chalk.cyan(
          'Signed off-chain order using EIP-712'
        )}. Signature: ${chalk.blue(signature)}, Signing Scheme: ${chalk.blue(
          signingScheme
        )}`
      )
    ...
    // Instantiate API and
    const safeApi = new SafeServiceClient({
      txServiceUrl: getGnosisSafeServiceUrl(chainId),
      ethAdapter,
    })
    ...
    console.log(
      `    ${chalk.bold('Threshold:')} ${chalk.blue(
        threshold
      )} out of ${chalk.blue(owners.length)}`
    )
    console.log(
      `    ${chalk.bold('Owners:')} ${chalk.blue(owners.join(', '))}\n`
    )
    ...
    if (postOrder) {
      ...
      const settlement = Settlement__factory.connect(
        settlementAddress,
        signerOrProvider
      )
      txs.push({
        ...
        txRequest: {
          ...
          value: '0',
          data: settlement.interface.encodeFunctionData('setPreSignature', [
            orderId,
            true,
          ]),
        },
      ...
      if (txTotal > 0) {
        console.log(
          `\n\n${chalk.cyan(
            `${chalk.red(txTotal)} Bundling Transactions`
          )}: Using Gnosis Safe\n`
        )
        let txNumber = 1
        for (const { txRequest, description } of txs) {
      ...
      const uiUrl = `https://gnosis-safe.io/app/${getSafeNetworkShortname(
        chainId
      )}:${fromAccount}/transactions/queue`
      console.log(
        `${chalk.cyan(
          '\nPropose Bundled Transaction'
        )}: In UI (${uiUrl})\n${JSON.stringify(safeTxProposal, null, 2)}`
      )
      await safeApi.proposeTransaction(safeTxProposal)
      console.log(
        `${chalk.cyan('🎉 Safe transaction has been created')}: See ${uiUrl}\n`
      )
      ...
      if (threshold === 1) {
        ...
        if (executeTransaction) {
          ...
          console.log(
            `${chalk.cyan(
              '🎉 Safe transaction has been sent'
            )}: Review in block explorer: ${chalk.blue(
              getExplorerUrl(chainId) + '/' + safeTxResult.hash
            )}`
          )
        } else {
          console.log(
            `${chalk.cyan(
              'OK remember someone will need to execute before the order expires'
            )}`
          )
        }
      } else {
        console.log(
          `${chalk.cyan(
            `Order created, but more signatures are required`
          )}: The order will need to be signed by other ${chalk.blue(
            threshold - 1
          )} signer(s)`
        )
      }
