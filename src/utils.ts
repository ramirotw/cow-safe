...
export const confirm = async (query: string): Promise<boolean> => {
  ...
  if (response === 'y' || response === 'Y') return true
    ...
    console.log(
      `${chalk.red(`Invalid response`)}: Please reply with a ${chalk.bold(
        'Y'
      )} or a ${chalk.bold('N')}`
    )
...
export function getSigner(
  ...
  switch (accoutType) {
    ...
    case 'SAFE_WITH_EOA_PROPOSER':
      ...
      assert(
        mnemonic,
        'MNEMONIC environment var is required for accountTypes EOA or SAFE_WITH_EOA_PROPOSER'
      )
...
export function getChainId(chainIdParam: number): ChainId {
  assert(
    SUPPORTED_CHAIN_IDS.includes(chainIdParam),
    'chainId must be one supported chainId. Supported: ' +
...
export function getChainIdFromEnv(): ChainId {
  ...
  assert(
    ...
    'CHAIN_ID must be one supported chainId. Supported: ' +
      SUPPORTED_CHAIN_IDS.join(', ')
  )
